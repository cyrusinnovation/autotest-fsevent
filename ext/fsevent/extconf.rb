...
if `uname -s`.chomp == 'Darwin'
  ...
  if ENV.has_key?('FSEVENT_SLEEP')
    ...
    #try to compile even without xcode...
    `CFLAGS='-isysroot' /usr/bin/gcc -framework CoreServices -o "#{GEM_ROOT}/bin/fsevent_sleep" fsevent_sleep.c`
    raise "\e[1;31mXcode not found - see README for assistance\e[0m" unless File.executable?("#{GEM_ROOT}/bin/fsevent_sleep")
  end
end
